...
function QuadTree(size, maxLeafs, maxDepth, x, y) {
    ...
    this.root = new Node(x, y, size, 0, maxDepth, maxLeafs);
}
QuadTree.Rect = Rect;
QuadTree.Node = Node;
QuadTree.Leaf = Leaf;
...
QuadTree.prototype._grow = function(leaf) {
    ...
    var d;
    if(lx < rx) {
        d = Math.ceil((rx - lx) / rw) * rw;
        rx -= d; rw += d;
    }
    if(ly < ry) {
        d = Math.ceil((ry - ly) / rh) * rh;
        ry -= d; rh += d;
    }
    if(lx + lw > rx + rw) {
        d = Math.ceil(((lx + lw) - (rx + rw)) / rw) * rw;
        rw += d;
    }
    if(ly + lh > ry + rh) {
        d = Math.ceil(((ly + lh) - (ry + rh)) / rh) * rh;
        rh += d;
    }
    ...
    var leafs = [leaf];
    this.root.getLeaf(leafs, this.root);
